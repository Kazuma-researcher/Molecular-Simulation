# 仮想Linux環境の構築
LinuxはオープンソースのOSであり、ソフト開発などの目的に応じてユーザーが自由にカスタマイズすることを想定して、最低限の機能だけを備えた土台としてのコンセプトを持つOSである。Linuxを土台として開発された様々な派生的なOSは**ディストリビューション**と呼ばれ、代表的なものとしてUbuntu、CentOS、Debianなどがある。Windowsがマウスを介したグラフィックベースの操作（GUI）をメインとするのに対し、Linuxはターミナルを介したコマンドベースの操作（CUI）がメインである。Linuxでもマウスによる操作は可能であるが、多くの場合コマンドによって操作を実行することになる。  

Linuxは上述のような柔軟性・カスタマイズ性に加え、高いセキュリティ性や安定性などの面から、サーバーOSとして採用されることが多い。**シミュレーションソフトは基本的にサーバーを介した大規模・高速計算を想定していることから、Linux向けに作られたものがほとんどである。** したがって、WindowsPCでMDシミュレーションを行うには、Windows上に仮想的なLinux環境を構築する必要がある。これはWindowsPCの中に仮想的なサブPCを作成し、その中にLinuxをインストールするようなイメージである。この仮想的なPCのことを**仮想マシン**と呼ぶ。

Windows11にはLinux環境を提供するための仮想マシンである**Windows Subsystem for Linux (WSL)** がデフォルトで入っており、これを利用した仮想Linux環境の構築を解説しているサイトが多い。筆者も最初はWSLを利用していたが、WSLからインターネットにアクセスするとファイアウォールに引っかかってしまうためプロキシを通す必要があることや、Windows updateのたびに仮想Linux環境が壊れてしまうなどの煩わしい面があったため、あまりおすすめしない。筆者はWSLに代わる仮想マシンとして、**VirtualBox**を使用した。VirtualBoxは使い方が非常に簡単で、その見た目から仮想マシンのイメージもつきやすいのでおすすめ。  
...
仮想環境の名前とOSを入力する画面になる。「名前」には"Ubuntu"などの環境名を付ける。「Folder」は仮想環境内で作成されたファイルの保存先と思われる。特に希望がなければそのままでOK。「ISO Image」は、先ほどダウンロードしたUbuntuのイメージファイルを選ぶ。これらの入力が終わったら「次へ」をクリック。  
...
## ホストマシンとの共有フォルダ作成
上述の作業で仮想Linux環境の構築と初期設定が完了したので、仮想環境上でシミュレーション練習を行うことができるようになった。これに加え、仮想マシンとホストマシンの間でファイルをやり取りできる共有フォルダを作っておくと、シミュレーションやデータ解析の結果を用いて書類を作成するときなどに便利である。共有フォルダの作成の手順を以下に記す。Ubuntuが起動している場合は一旦終了しておく。  

ホストマシン側の任意の場所に共有フォルダを作成する。

![タイトルなし](https://github.com/user-attachments/assets/26792c29-3b33-4e1c-9fce-723142cf0534)

VirtualBoxを開き、「設定」ボタンをクリック。

![タイトルなし](https://github.com/user-attachments/assets/9d2ca0fb-53ed-47e9-971c-88c9274f4f9d)

「共有フォルダー」を開き、新規共有フォルダー作成ボタンをクリック。

![タイトルなし](https://github.com/user-attachments/assets/a60d8135-f370-44d3-b808-d6b8fcf9ec89)

「フォルダーのパス」に先ほど作成した共有フォルダーを選択し、「フォルダー名」を入力。「自動マウント」にチェックを入れて「OK」をクリック。

![タイトルなし](https://github.com/user-attachments/assets/df9a9b46-3aed-44dc-a516-b1bd9cf4a448)

以上で共有フォルダの作成が完了。Ubuntuを起動し、左メニューから「ファイル」を開くと、共有フォルダが作成されているはずである。Ubuntu内の共有フォルダの名前は「sf_@@@」となっており、@@@が先ほど設定したフォルダ名である。

![タイトルなし](https://github.com/user-attachments/assets/9eabb7b0-cdaf-40fb-a5dc-fb3901bdabf9)

Ubuntu上で共有フォルダにファイルを置くためには、ターミナルからコマンドで操作を実行する必要がある。Ubuntuのコマンド解説については別途ドキュメントを作成する。
